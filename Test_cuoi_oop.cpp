...
class TacGia{
  ...
  public:
    friend class SachGK;
       friend void TimSachTheoTenNXB(SachGK *ds, int n, const char *tenNXB,const char *tentg);
};
...
void TimSachTheoTenNXB(SachGK *ds, int n, const char *tenNXB,const char *tentg){
  bool check = false;
  for(int i = 0; i < n; i++){
    if((strcmpi(ds[i].y.tenNXB, tenNXB) == 0)&&(strcmpi(ds[i].x.tenTacGia, tentg)==0)){
      check = true;
      break;
    }
  }
...
main(){
  ...
  DanhSachSachGK(ds, n);
  TimSachTheoTenNXB(ds, n, "Giao Duc","Nguyen Van At");
